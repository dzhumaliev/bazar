package solutions.isky.gaurangarevolution.slider.library.Indicators;
...
import solutions.isky.gaurangarevolution.slider.library.SliderAdapter;
...
public class PagerIndicator extends LinearLayout implements ViewPagerEx.OnPageChangeListener{
    ...
    public void redraw(){
        ...
        for(int i =0 ;i< mItemCount+mItemCount-1; i++){
            if(i% 2 == 0){
                ...
                lp.gravity=(i % 4 == 0)? Gravity.TOP:Gravity.BOTTOM;
                mIndicators.add(indicator);
            }else{
                ImageView liner = new ImageView(getContext());
                liner.setImageResource(R.drawable.divider_indikator);
                if((i+1)%4==0){
    ...
    private DataSetObserver dataChangeObserver = new DataSetObserver() {
        ...
        public void onChanged() {
            ...
            if(count+count-1 > mItemCount){
                for(int i =0 ; i< count+count-1 - mItemCount;i++){
                    ImageView indicator = new ImageView(mContext);
                    indicator.setImageDrawable(mUnselectedDrawable);
                    indicator.setPadding((int)mUnSelectedPadding_Left,
                        (int)mUnSelectedPadding_Top,
            ...
            }else if(count+count-1 < mItemCount){
                for(int i = 0; i < mItemCount - count+count-1;i++){
                    removeView(mIndicators.get(0));
                    mIndicators.remove(0);
                }
            }
            mItemCount = count+count;
            mPager.setCurrentItem(mItemCount*20 + mPager.getCurrentItem()+mPager.getCurrentItem());
        }
    ...
    private void setItemAsSelected(int position){
        ...
        ImageView currentSelected = (ImageView)getChildAt(position + 2);
    ...
    public void onPageSelected(int position) {
        ...
        setItemAsSelected(position+position-2);
    }
    @Override
