...
abstract class Prefs<K extends BaseKey> {
    ...
    public final float getFloat(K key) {
        checkKey(key, Float.class);
        return mPrefs.getFloat(key.toString(), (Float) key.getDefaultValue());
    }

    public final void setFloat(K key, float value) {
        checkKey(key, Float.class);
        mPrefs.edit().putFloat(key.toString(), value).apply();
    }
