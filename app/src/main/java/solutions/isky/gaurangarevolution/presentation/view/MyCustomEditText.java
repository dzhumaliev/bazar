...
public class MyCustomEditText extends AppCompatEditText {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (hasFocused && mBitmap != null && isPwdInputType && !isTextEmpty()) {
            int left = getMeasuredWidth() - getPaddingEnd() -
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (hasFocused && isPwdInputType && event.getAction() == MotionEvent.ACTION_UP) {
            ...
            int right = getMeasuredWidth() - getPaddingEnd();
            boolean isAreaX;
            if(Locale.getDefault().getLanguage().equals("ar")){
                isAreaX = event.getX() <= w && event.getX() >= 0;
            }else{
                isAreaX = event.getX() <= right && event.getX() >= right - w;
            }
            ...
            if (isAreaX && isAreaY) {
                ...
                setCompoundDrawablesRelative(getCompoundDrawablesRelative()[0], getCompoundDrawablesRelative()[1],
                        mTogglePwdDrawable, getCompoundDrawablesRelative()[3]);
            ...
            if (!disableClear) {
                right -= w + dp2px(4);
                if(Locale.getDefault().getLanguage().equals("ar")){
                    isAreaX = event.getX() <= (w + dp2px(4)+mBitmap.getWidth()) && event.getX() >= w;
                }else{
        ...
        if (hasFocused && !disableClear && !isPwdInputType && event.getAction() == MotionEvent.ACTION_UP) {
            ...
            boolean isAreaX;
            if(Locale.getDefault().getLanguage().equals("ar")){
                isAreaX = event.getX() <= rectW && event.getX() >= 0;
            }else{
                isAreaX = event.getX() <= right && event.getX() >= right - rectW;
            }
...
//                }
    ...
    private void markerFocusChangeLogic() {
        if (!hasFocused || (isTextEmpty() && !isPwdInputType)) {
            setCompoundDrawablesRelative(getCompoundDrawablesRelative()[0], getCompoundDrawablesRelative()[1],
                    null, getCompoundDrawablesRelative()[3]);
            ...
            if (isPwdInputType) {
                ...
                setCompoundDrawablesRelative(getCompoundDrawablesRelative()[0], getCompoundDrawablesRelative()[1],
                        mClearDrawable, getCompoundDrawablesRelative()[3]);
            }
        }
    }
