...
public class NiceSpinner extends AppCompatTextView {
    ...
    @Override public void onRestoreInstanceState(Parcelable savedState) {
        if (savedState instanceof Bundle) {
            ...
            if (adapter != null) {
                setTextInternal(((Currency)adapter.getItemInDataset(selectedIndex)).getTitleShort());
    ...
    public int getSelectedCurrencyID() {
        try {
            return ((List<Currency>)adapter.getlist()).get(selectedIndex).getId();
        }catch (Exception e){
            return 0;
        }
    ...
    public void setSelectedID(int id) {
        if (adapter != null) {
            for(int i=0;i<((List<Currency>)adapter.getlist()).size();i++)
                if(((List<Currency>)adapter.getlist()).get(i).getId()==id){
